...
export class InputRestrictionDirective {
  ...
  @HostListener('keypress', ['$event']) onKeyPress(event) {
    ...
    else if (this.appInputRestriction === 'noSpecialCharsslash') {
      this.noSpecialCharsslash(event);
    }
  }
  ...
  noSpecialCharsslash(event) {
    ...
    let k;
    k = event.keyCode;  // k = event.charCode;  (Both can be used)
    if ((k > 64 && k < 91) || (k > 96 && k < 123) || k === 8 || k === 32 || k===46 || (k >= 48 && k <= 57) || k===47 || k===45) {
